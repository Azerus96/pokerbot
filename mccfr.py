...
class MCCFR:
    def __init__(self, player, iterations=1000):
        """
        Инициализация MCCFR.
        player: объект игрока, которому принадлежит MCCFR
        iterations: количество итераций для тренировки (по умолчанию 1000)
        """
        self.player = player
        self.iterations = iterations
        self.strategy = defaultdict(lambda: [0, 0])  # Для хранения стратегии: [регреты, кумулятивная стратегия]
    ...
    def save_strategy(self, file_name=None):
        """
        Сохранение MCCFR стратегии в файл, чтобы загрузить позже или перенести на другой турнир.
        file_name: имя файла (по умолчанию создаётся имя из имени игрока)
        """
        if file_name is None:
            file_name = f'{self.player.name}_mccfr_strategy.pkl'
        with open(file_name, 'wb') as f:
            pickle.dump(self.strategy, f)
        print(f"Стратегия игрока {self.player.name} сохранена в файл: {file_name}")
    ...
    def load_strategy(self, file_name):
        ...
        print(f"Стратегия игрока {self.player.name} загружена из файла: {file_name}")
