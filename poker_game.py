...
class PokerGame:
    ...
    async def conduct_betting_round(self, table, stage):
        """Проведение одного круга ставок"""
        for player in table:
            game_state = {
    ...
    async def play_round(self):
        ...
        # Определяем новое распределение игроков (если кто-то вылетел)
        self.reorganize_tables()

        # Обновляем состояние на фронте
        tournament_state = {
    ...
    def get_table_state(self):
        """Метод для получения текущего состояния стола (отправляется на фронт)"""
        tables_state = []
